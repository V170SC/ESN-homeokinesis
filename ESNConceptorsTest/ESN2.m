...
% for k=1:100
stream = RandStream('mt19937ar','Seed',66);
...
Win = (rand(resSize, 1+inSize)-0.5)*2;
W = (rand(resSize)-0.5)*2;
% W = (sprand(resSize,resSize,10/resSize)-0.5)*2;
Wback = (rand(resSize,1+outSize)-0.5)*2;
...
M = zeros(inSize+outSize+resSize,train);
...
Wout = T*M'*inv(M*M'+reg*eye(inSize+outSize+resSize));

Y = zeros(outSize,test);
u = output(train+init+1);
y = 0;
for t = 1:test
	% x = (1-a)*x + a*tanh( Wback*[1;u] + W*x + wgn(resSize,1,0)/10000);
	x = (1-a)*x + a*tanh( Wback*[1;u] + W*x);
	y = Wout*[1;u;x];
	Y(:,t) = y;
	u = y;
