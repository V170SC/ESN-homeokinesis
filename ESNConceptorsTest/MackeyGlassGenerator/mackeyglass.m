...
%%
% 
% $$\frac{dx(t)}{dt}=\frac{ax(t-\tau)}{1+x(t-\tau)^{n}}-bx(t)
% \hspace{1cm} (1)$$
% 
...
%%
% 
% $$x(t+\Delta t) = mackeyglass\_rk4(x(t), x(t-\tau), \Delta t, a, b, n)$$
%
...
%% Input parameters
a        = 0.2;     % value for a (beta) in eq (1)
b        = 0.1;     % value for b (gamma) in eq (1)
tau      = 17;		% delay constant in eq (1)
n		 = 10;    % exponent of x in eq (1)
...
for i = 1:sample_n+1,
    ...
    x_t_plus_deltat = mackeyglass_rk4(x_t, x_t_minus_tau, deltat, a, b, n);
